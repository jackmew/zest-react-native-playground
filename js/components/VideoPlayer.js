...
export default class VideoPlayer extends Component {
  ...
  renderSubtitles() {
    const subtitles = [
      { time: 120, content: 'Image: React Native provides a unified way of managing images and other media assets in your iOS and Android apps. To add a static image to your app, place it somewhere in your source code tree and reference it like this:' },
      { time: 280, content: 'Using ScrollView: The ScrollView is a generic scrolling container that can host multiple components and views. The scrollable items need not be homogenous, and you can scroll both vertically and horizontally (by setting the horizontal property).' },
      { time: 430, content: 'FlatList: The ScrollView is a generic scrolling container that can host multiple components and views. The scrollable items need not be homogenous, and you can scroll both vertically and horizontally (by setting the horizontal property).' }
    ];
    return subtitles.map((subtitle, index) => {
      return (
        <TouchableOpacity key={index} style={{ margin: 10, paddingLeft: 10, borderLeftWidth: 5, borderLeftColor: 'lightblue' }} onPress={() => { this.player.seek(subtitle.time); }}>
